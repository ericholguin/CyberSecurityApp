...
class ExampleApp(QMainWindow, design.Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        def openTab():
            ...
            treeStr = str(self.treeWidget.currentItem())
            treeStr2 = "af8"
            treeStr3 = "b88"
            treeStr4 = "c18"
            treeStr5 = "ca8"
            ...
            treeStr11 = "o048"
            treeStr12 = "0d8"
            if (treeStr.find(treeStr2) != -1):
                self.tabWidget.addTab(self.infoGatheringTab, "Information Gathering")
            elif (treeStr.find(treeStr3) != -1):
                 self.tabWidget.addTab(self.acccheckTab, "acccheck")                
            elif (treeStr.find(treeStr4) != -1):
                 self.tabWidget.addTab(self.acevoipTab, "ace-voip")
            elif (treeStr.find(treeStr5) != -1):
                 self.tabWidget.addTab(self.amapTab, "amap")
            elif (treeStr.find(treeStr7) != -1):
                 self.tabWidget.addTab(self.automaterTab, "Automater")
            elif (treeStr.find(treeStr8) != -1):
                 self.tabWidget.addTab(self.bingip2hostsTab, "bing-ip2hosts")
            elif (treeStr.find(treeStr9) != -1):
                 self.tabWidget.addTab(self.braaTab, "braa")

        self.treeWidget.itemClicked.connect(openTab)

        # if (self.Click1.isSelected() == True):
        #     {
        #         self.treeWidget.itemClicked(self.Click1, 0).connect(openTab)
        #         #self.connect(self.tabWidget.addTab(self.acccheckTab, "acccheck")
        #     }
